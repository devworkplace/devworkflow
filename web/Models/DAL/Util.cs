...
namespace web.Models.DAL
{
    public static class Util
    {
        ...
        public static string CONNECTION_STRING = WebConfigurationManager.ConnectionStrings["hektor-flow"].ConnectionString;
        public const int CODIGO_TIPO_VALOR_COMPONENTE_DEFAULT = 1;

        private static String FORMATO_DATA_HORA_FILTRO
        {
            get
            {
                Object result = HttpContext.Current.Application[ParametroDAL.FORMATO_DATA_HORA_FILTRO];
                if (null == result || String.IsNullOrEmpty(result.ToString()))
                {
                    Parametro formatoDataHoraParaFiltro = ParametroDAL.GetPorNome(ParametroDAL.FORMATO_DATA_HORA_FILTRO);
                    if (null == formatoDataHoraParaFiltro)
                    {
                        result = "PT-BR";
                    }
                    else
                    {
                        result = formatoDataHoraParaFiltro.VALOR;
                    }
                    HttpContext.Current.Application[ParametroDAL.FORMATO_DATA_HORA_FILTRO] = result;
                }
        ...
        public static String GetDataHoraParaFiltro(String hora, String[] camposData)
        {
            switch (Util.FORMATO_DATA_HORA_FILTRO)
            {
                case "PT-BR":
                    return String.Format("{0}-{1}-{2} {3}", camposData[0], camposData[1], camposData[2], hora);
                default:
                    return String.Format("{0}-{1}-{2} {3}", camposData[2], camposData[1], camposData[0], hora);
            }
        }
    }
}